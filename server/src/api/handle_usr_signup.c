...
void handle_usr_signup(const cJSON* user_info, t_server_utils* utils) {
    ...
    if (database_init() != 0) {
        send_server_response(utils->client_socket, R_DB_FAILURE, REQ_USR_SIGNUP);
        return;
    }
    ...
    if (!cJSON_IsString(user_name) || !cJSON_IsString(user_password)) {
        send_server_response(utils->client_socket, R_JSON_FAILURE, REQ_USR_SIGNUP);
        return;
    }

    if (db_user_exists(user_name->valuestring)) {
        send_server_response(utils->client_socket, R_USR_EXISTS, REQ_USR_SIGNUP);
        return;
    }

    if (!is_name_format_valid(user_name->valuestring)) {
        send_server_response(utils->client_socket, R_NAME_FORMAT_INVALID, REQ_USR_SIGNUP);
        return;
    }
    ...
    send_server_response(utils->client_socket, R_SUCCESS, REQ_USR_SIGNUP);
